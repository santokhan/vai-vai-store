          ...
          <thead>
            <tr className="bg-gray-100">
              {columns.map((column) => (
                <th
                  key={column.id}
                  className="p-2 text-start font-medium uppercase"
                >
            ...
            {rows.map((row) => (
              <tr key={rowKey(row)}>
                {columns.map((column) => (
                  <td
                    key={column.id}
                    className="p-2 whitespace-nowrap capitalize"
                  >
        ...
        <div className="py-4 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <button
              className="border rounded-lg p-2"
              onClick={() =>
                setPageIndex((page) => Math.min(page + 1, pageCount - 1))
              }
            ...
            <PageOutOf
              ...
              onPageChange={(page) =>
                setPageIndex(Math.min(Math.max(page, 1), pageCount) - 1)
              }
            />
          </div>
